...
- **Qualification override для rollback/fallback**: утверждение выше о переходе `RolledBack → Serving` применимо только когда persisted `configuredProfileId` входит в `qualifiedProfileIds` загруженного ранее committed image. Сам факт предыдущего commit не квалифицирует image для профиля, выбранного позже. При несовместимости приложение оставляет `activeProfileId` отсутствующим и входит в `Recovery` с `ProfileQualificationMismatch` по §11; motion закрыт до установки совместимого signed image.
