...
Статус: **все решения владельца приняты (Q1–Q8; D_accept = O3; Q5 = A; F2 — внешние ограждения + процесс, без интерлока); независимое экспертное ревью проведено — замечания F1–F10 учтены (F1–F6, F9 — решения владельца; F7, F8, F10 — правки записи); каталог — first-pass (ревью на G2); приёмка O3 — вилка C1b разрешена в #48 (D_accept = 1.0 м + правило снижения maxSpeed).** Финализация — при закрытии issue #45. Вход в logical item `Safety & Health` нормативного пакета (issue 8, gate G2) и в item `Quality Attributes & Budgets` (тикет #48).
...
Каталог построен из V1-свидетельств ([системный индекс](./research/v1-system-evidence-index.md)) и walkthrough operating contexts (issue 2) × интерфейсы. Требует ревью при формировании пакета (G2).
...
| HZ-16 | все контексты × ADC, BMS | перегрев MCU/батареи | повреждение, отказ | L (confidence: low) | ATEMP (V1), BMS-температура (детали неизвестны) | мониторинг, low-battery реакции | Degraded | warning → stop по порогам | охлаждение → recovery | перегрев до 110 °C fault при T_j,max 125 °C — запас 15 °C, принят владельцем (#48) | host: health FSM; пороги — #48 | owner (решено в #48: warn 90 °C / fault 110 °C, гистерезис 5 °C; residual — запас 15 °C до max junction 125 °C, принят владельцем) |
| HZ-17 | все контексты × BMS (RS-485) | потеря наблюдения BMS → неизвестный SOC | глубокий разряд, застревание в проходе | M (confidence: low — нет полевых данных) | BMS staleness после T_bms_stale (V1: только warning — BmsDdA5.hpp:74-75, Cntrl_V2.ino:7702-7706) | warning + событие + лог; работа продолжается — **явный acceptance владельца (F9)** | Degraded (информационно) | warning, продолжение рабочего процесса | восстановление BMS-связи | глубокий разряд — принятый риск (покрытие внешним BMS-контроллером не подтверждено) | host: staleness-логика; T_bms_stale = 60 s (#48) | owner (accepted, F9) |
...
Authority остаточных рисков — **владелец, через G2 gate** (issue 8: owner approval принимает residual risks). Форма — Governance risk-acceptance записи: статусы `accepted` / `mitigated` / `open-obligation` (с датой) per hazard в поле `acceptance` схемы. Unknown-статусы на G2 не допускаются: каждый hazard закрыт обязательством (с владельцем) или явным acceptance. Verification obligations — записи класса Evidence (issue 8), обязательны до G2 или явно отклонены с основанием. Спец-случаи: HZ-03 `mitigated` (Q7.1 A); HZ-16 `accepted` (пороги warn 90 °C / fault 110 °C — решено в #48, запас 15 °C до max junction 125 °C принят владельцем); HZ-01 `accepted` при **O3** (вилка C1b разрешена в #48: D_accept = 1.0 м + правило снижения maxSpeed — F1 ревью).
...
- **Degraded**: движение ограничено по capability-классам (конкретные ограничения — item 4 `Capability & Operation Contracts`; здесь: движение только при выполнении motion-инвариантов INV-SENSING-FRESH и INV-OBSTACLE-CLASSIFY). Вход — degraded-условия (HZ-05/06/13/14/16); выход — снятие условия. Событие + warning, никогда молча. **F5 (ревью, решение владельца):** в Degraded действует **потолок скорости ≤ 1.0 м/с** (утверждено владельцем); временной bound `T_deg = 60 s` непрерывного пребывания в Degraded ⇒ Fault/stop — **утверждён владельцем (#48, Q2)**; fault-код `FAULT_DEGRADED_TIMEOUT` — в wire-реестр #47; ограничение не откладывается в item 4.
...
- **Auto-clear eligible** (квалификация + stationary + отсутствие motion intent): sensing/bus faults — ToF, AS5600, I2C bus (HZ-01/05/06). Квалификация — V1-базелайн: sustained fresh/usable streak; окна и пороги — measurement-resolved (proving slice #54 + полевые тесты, план #48). Владелец механики — адаптеры, health state machine — Sensing.
...
**Критерий владельца — РЕШЕНО: O3, узкая запись (F1 ревью).** Решение владельца: `T_fresh = 300 ms` фиксированный (V1 keep). **C1 разрешён в #48:** полная дистанция остановки `D_accept = 1.0 м` (цепочка + D_brake); правило вилки C1b — при измеренном пустом интервале (T_sample_worst + margin ≥ 300 ms или цепочка + D_brake > D_accept при v_max) применяется **снижение maxSpeed** (порт-предпочтение), расширение D_accept — только с геометрическим обоснованием и impact review. Члены цепочки (T_check_jitter, T_arb, T_emit) и D_brake — measurement-resolved (раздел 7.2). **Порт-предпочтение владельца:** минимальное отклонение от V1-кода (существующие конфигурационные лимиты maxSpeed, 300 ms) — для простоты портирования. O1 (100 мм) и O2 (200–300 мм) отклонены. Скорость-масштабирование не применяется (V1 фиксированный порог).
...
| LSI (IWDG clock) | 17 / 32 / 47 kHz (min/typ/max). Для конфигурации V1 (`IWatchdog.begin(10000000)` ⇒ prescaler 128, reload 2499 при LSI_VALUE=32000) аппаратный timeout: **6.8 s @47 kHz … 18.8 s @17 kHz** (номинал 10 s @32 kHz). Макс. конфигурация (PR 256, RL 4095): 22.3…61.7 s. Бюджетировать reload на fast-конец 47 kHz (6.8 s), backstop-латентность — на slow-конец 17 kHz | DS8626 Table 35; RM0090 Table 107; IWatchdog.cpp `set()` (проверено по исходнику, #48) | — | primary + derived |
...
Закрываются измерениями proving slice (#54) и тестами на реальном шаттле в процессе переноса функционала (решение владельца; план и бюджеты — #48):
...
- **Решено:** численные пороги тепловых реакций (HZ-16) — warn 90 °C / fault 110 °C, гистерезис 5 °C (тикет #48); детекция — ATEMP, владелец — #50 (сервисные режимы).
