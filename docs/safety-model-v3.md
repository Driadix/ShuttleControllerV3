...
- **Degraded**: движение ограничено по capability-классам (конкретные ограничения — item 4 `Capability & Operation Contracts`; здесь: движение только при выполнении motion-инвариантов INV-SENSING-FRESH и INV-OBSTACLE-CLASSIFY). Вход — degraded-условия (HZ-05/06/13/14/16); выход — снятие условия. Событие + warning, никогда молча. **F5 (ревью, решение владельца):** в Degraded действует **потолок скорости ≤ 1.0 м/с** и максимальная длительность `T_deg` → Fault/stop (число — #48); ограничение не откладывается в item 4.
