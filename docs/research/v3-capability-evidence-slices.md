...
- **Version**: bundle revision `8dd3d0c` -> `70d78ec` (ветка `research/v3-capability-evidence-slices`, перенесена в пакет issue 53; ревизия защищена тегом `evidence/v3-capability-evidence-slices`); независимый checklist-review по gate `legacy evidence ready` пройден, 5 minor-правок anchors внесены, блокирующих замечаний нет.
...
- **Retention obligation (commit-pinned links)**: выполнена (issue 55) - системный индекс и execution evidence промоутнуты в `main` (`docs/research/v1-system-evidence-index.md`, `docs/research/v1-execution-evidence.md`), ссылки ниже переведены на относительные; research-ветки `research/v1-system-evidence` и `research/v1-execution-evidence` больше не являются источником доступа к evidence и могут быть удалены без 404. Исторические commit-pinned ссылки в issue-текстах защищены lightweight tags `evidence/v1-system-evidence-index`/`evidence/v1-execution-evidence` (provenance-требование issue 5 соблюдено).
- **Evidence-integrity (U+2014)**: evidence-файлы промоутнуты verbatim (индекс отличается от `a7f927c` одной disclosed-правкой MD034; execution evidence идентичен `22b8990`); предсуществующие U+2014 в контенте индекса оставлены как есть (dash-sweep - отдельное решение владельца).
