...
- **Fact**: pre-flight IDR-проверка шины УДАЛЕНА: на этом стенде SDA/SCL читаются как LOW (IDR биты 10/11 = 0) при работающей прошивке и HIGH при halt, при этом транзакции проходят в обоих случаях (evidence 2026-08-14) - IDR-уровень не является индикатором stuck. Stuck классифицируется только по исходу Wire/HAL (tx=1/4/5). Recovery (ручной ≤16 SCL + STOP + reinit) сохранён для реального stuck.
- **Fact**: инициализация I2C - V1-стиль (`Wire.setSDA(PB11); Wire.setSCL(PB10); Wire.begin(); Wire.setClock(100000)`), подтверждённый эмпирически на стенде 2026-08-13: форма `Wire.begin(PB11, PB10)` давала tx=4 (Stuck) на той же шине с теми же пинами (GPIO при variant 0: AF4/open-drain/pull-up/IDR=1 - пины настроены корректно, шина жива), а V1-стиль читает датчики (0x09/0x0C Healthy, AS5600 Healthy, ToF-чтение 1479 µs). Причина расхождения в ядре 4.20701.0 не установлена (begin(sda,scl) де-факто идентичен setSDA/setSCL+begin по коду) - зафиксирован как факт, не как модель; повторно не исследовать (V1-стиль нормативный, работает).
